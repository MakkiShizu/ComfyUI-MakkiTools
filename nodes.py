...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "ImageChannelSeparate": "ImageChannelSeparate(mki-图像通道分离)",
    "ImageCountConcatenate": "ImageCountConcatenate(mki-图像批次拼接)",
    "MergeImageChannels": "MergeImageChannels(mki-图像通道合并)",
    "ImageWidthStitch": "ImageWidthStitch(已弃用-mki-图像横向拼接)",
    "ImageHeigthStitch": "ImageHeigthStitch(已弃用-mki-图像纵向拼接)",
    "AutoLoop_create_pseudo_loop_video": "AutoLoop_create_pseudo_loop_video(已弃用-mki-创建伪循环视频)",
    "Environment_INFO": "Environment_INFO(mki-环境信息显示)",
    "translators": "translators(mki-多语言翻译)",
    "translator_m2m100": "translator_m2m100(mki-多语言翻译-m2m100)",
    "random_any": "random_any(mki-随机数输出)",
    "AnyImageStitch": "AnyImageStitch(mki-任意数量图像拼接)",
    "AnyImagetoConditioning_flux_kontext": "AnyImagetoConditioning_flux_kontext(mki-任意数量图像转条件-flux_kontext)",
    "show_type": "show_type(mki-数据类型显示)",
    "timer": "timer(mki-计时器)",
